...
namespace CrashCapture {
        ...
        while (pos + (uintptr_t)len <= end) {
            if (pos + (uintptr_t)len > readableEnd) {
                ...
                while (readableEnd < pos + (uintptr_t)len) {
                    ...
                    if (!Mem_IsReadable((void*)pageStart, 1)) {
    ...
    {
        const CCModule* m = module ? Modules_FindByName(module) : NULL;
        if (!m) return 0;
        CCPattern pat;
        if (!Sig_Compile(ida, &pat)) return 0;
        return Sig_Find(m, &pat);
    }

    // --------- cc-sig-resolve ---
    uintptr_t Sig_RelTarget(uintptr_t at, int opOff, int insnLen)
    {
        uintptr_t opnd = at + (uintptr_t)opOff;
        if (!Mem_IsReadable((void*)opnd, 4)) return 0;
        int32_t disp;
        memcpy(&disp, (void*)opnd, 4);
        return at + (uintptr_t)insnLen + (uintptr_t)(intptr_t)disp;
    }

    uintptr_t Sig_Abs32(uintptr_t at, int opOff)
    {
        uintptr_t opnd = at + (uintptr_t)opOff;
        if (!Mem_IsReadable((void*)opnd, 4)) return 0;
        uint32_t v;
    ...
    uintptr_t Sig_Deref(uintptr_t at)
    {
        if (!Mem_IsReadable((void*)at, sizeof(void*))) return 0;
        void* p;
    ...
    uintptr_t Sig_Resolve(const CCTarget* t)
    {
        ...
        if (t->symbol && t->symbol[0]) {
            cur = Sym_Lookup(t->module, t->symbol);
            if (!cur && t->module) cur = Sym_Lookup(NULL, t->symbol); // fall back to global search
        }
        if (!cur) {
            if (!t->sig || !t->sig[0]) return 0;

            const CCModule* m = t->module ? Modules_FindByName(t->module) : NULL;
            if (!m) return 0;

            CCPattern pat;
            if (!Sig_Compile(t->sig, &pat)) return 0;
        ...
        for (int i = 0; i < 4 && t->steps[i].op != CC_STEP_END; ++i) {
            ...
            switch (s.op) {
                case CC_STEP_REL: cur = Sig_RelTarget(cur, s.a, s.b); break;
                case CC_STEP_ABS32: cur = Sig_Abs32(cur, s.a); break;
                case CC_STEP_DEREF: cur = Sig_Deref(cur); break;
    ...
    void Sig_Init()
    {
        for (int t = 0; t < g_nTables; ++t)
            for (int i = 0; i < g_tableCount[t]; ++i) {
                ...
                CacheStore(tg->key, Sig_Resolve(tg));
            }
    }
