...
namespace CrashCapture {
    ...
    void Phys::Bind::Init()
    {
        Sig::Register(kHookTargets, (int)(sizeof(kHookTargets) / sizeof(kHookTargets[0])));
            ...
            Patch::Register(kPhysPatches, (int)(sizeof(kPhysPatches) / sizeof(kPhysPatches[0])));
        #endif
    }
    ...
    static uint32_t h_minlist_add(void* selfPtr, void* elem, float value)
    {
        ...
        *(uint16_t*)(e + kMinListElemLongNext) = kMinListLongUnused;
        ...
        if (!g_minlistSkipList && countCmp > maxCmpLen) {
            uint32_t newLongPos = firstjAfter;
            for (uint32_t k = 2; k < maxCmpLen; ++k)
                newLongPos = *(uint16_t*)(MinListSlot(elems, newLongPos) + kMinListElemNext);
    ...
    void Phys::Bind::RefreshToggles()
    {
        g_minlistSkipList = Patch::Enabled("gm.phys.minlist_skip_list");
        g_minlistBoundA = Patch::Enabled("gm.phys.minlist_walk_bound_a");
        g_minlistBoundB = Patch::Enabled("gm.phys.minlist_walk_bound_b");
        g_minListMalloc = Sig::Get("patch.p_malloc");
        g_minListFree = Sig::Get("patch.p_free");
    }
